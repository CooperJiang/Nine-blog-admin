    ...
    <el-card class="box-card">
        <template #header>
            <div class="flex_between">
                <span>分类列表</span><el-button type="text" @click="addType">添加工具分类</el-button>
            </div>
        </template>
        ...
        <div class="table">
            <el-table :data="tableData" style="width: 100%;" border>
                ...
                <el-table-column prop="tools_num" label="分类数量" align="center" width="150" />
        ...
        <el-dialog v-model="visible" :title="titleMsg" width="400px">
            <el-form ref="formRef" :model="formData" :rules="rules">
                <el-form-item label="分类名称" prop="name" label-width="90px">
                    <el-input v-model="formData.name" placeholder="请填写资源分类名称" @keyup.enter="submit" />
                </el-form-item>
...
import { getToolsType, setToolsType, delToolsType } from '@/api/tools/tools-type'
import { ElMessage } from 'element-plus'
import {formatTime} from '@/util/tools'
import {statusList} from '../../resource/constant'
...
async function queryType() {
    const {rows, count} = await getToolsType(data.params)
    formatTime(rows)
    data.tableData = rows
    data.total = count
}
...
function handleEdit(row) {
    ...
    data.activeId = id
    data.type = 2
    console.log({name, status, orderId })
    Object.assign(data.formData, {name, status, orderId })
    data.visible = true
}

async function deleteType(id) {
    await delToolsType({id})
